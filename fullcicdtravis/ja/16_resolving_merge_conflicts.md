...
> このセクションのリモートマージコンフリクトは、まだ**準備されていません**。 ファシリテーターが休憩の間、次の課題に進むために、上流のリポジトリに変更を加えましょう。
> 
> 1. 上流のリモート（githubschool/github-games）を編集する準備をします。
> 2. `shape-colors` のブランチをチェックアウトします。
> 3. `index.html` のファイルを見つけます。
> 4. 115 - 121行目で色を変更します。
> 
> この変更は、クラスでリモートを追加する前に行いましょう。 リモートの編集作業を飛ばしてしまうと、クラス全員が各々のリポジトリで `shape-colors` を変更しなければいけなくなります。 {: .facilitator} {% endif %}
